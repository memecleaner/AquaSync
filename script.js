...
// =================================================================
const URI_VISUAL_STANDBY = "clips/zzz_icon.png"; // Kosongkan agar saat OFF tetap pakai emoji 💤
const URI_VISUAL_VERIFIKASI = "clips/hourglass.png"; // 🔥 JALUR FILE PNG BERGERAK ANDA
const URI_VISUAL_AKTIF = ""; // Kosongkan agar saat ON otomatis pakai emoji 🌀⚡
    ...
    if (data) {
        ...
        if (data.Pump_Button === 1) {
            ...
            if (isVibrationValidated) {
                ...
                // Panggil fungsi injector untuk mode AKTIF
                ...
                // 🔥 JALUR UTAMA SUNTIK GAMBAR BERGERAK PAS LOADING VERIFIKASI SENSOR GETAR
            ...
            // Panggil fungsi injector untuk mode OFF / STANDBY
...
// B. SINKRONISASI DATA KELISTRIKAN SENSOR PZEM
...
window.submitWaterPurpose = function(purpose) {
    ...
    if (currentPumpState === 1 && globalPumpTimeout > now) {
        ...
        set(ref(database, 'AquaSync/Realtime_Status/Vibration'), false)
        .then(() => {
            updates['AquaSync/Realtime_Status/Pump_Button'] = 1;
            updates['AquaSync/Realtime_Status/Active_User'] = finalUsers;
            updates['AquaSync/Realtime_Status/Water_Purpose'] = finalPurposes;
...
// 🔥 FUNGSI RE-DRAW INTERPRETER: MENTERJEMAHKAN ALAMAT FILE JADI TAG <IMG>
function suntikVisualDinamis(elementTarget, uriGambar, emojiSerep) {
    ...
    if (uriGambar && uriGambar !== "") {
        // Jika parameter gambar diisi, paksa cetak tag HTML <img> ke dalam halaman web
        ...
        // Jika dikosongkan, kembalikan ke wujud teks karakter emoji aslinya
